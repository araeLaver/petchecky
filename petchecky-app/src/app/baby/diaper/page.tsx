...
import DiaperTracker from "@/components/baby/DiaperTracker";
import type { DiaperRecord } from "@/types/baby";

const STORAGE_KEY = "petchecky-baby-diaper";

function getRecords(): DiaperRecord[] {
  if (typeof window === "undefined") return [];
  const data = localStorage.getItem(STORAGE_KEY);
  return data ? JSON.parse(data) : [];
}

function saveRecords(records: DiaperRecord[]) {
      ...
      <header className="sticky top-0 z-10 border-b border-gray-100 bg-white/80 backdrop-blur-sm dark:border-gray-800 dark:bg-gray-900/80">
        <div className="mx-auto flex h-14 max-w-3xl items-center justify-between px-4">
          <Link href="/baby" className="flex items-center gap-2 text-gray-600 hover:text-gray-900 dark:text-gray-400">
            ...
            <span className="font-bold text-gray-800 dark:text-gray-100">👶 기저귀 기록</span>
          </Link>
        </div>
      </header>
